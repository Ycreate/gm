...
/*:
 * @plugindesc SAN_MapGenerator ver1.10
 ...
 * 
 * @param WallHight
 * @desc Set wall height. (1-3)
 * @default 1
 * 
 * @param MinRoomSize
 * @desc Set minimum room size. (3-)
 * @default 3
 * 
 * @param MaxRoomSize
 * @desc Set maximum room size. (3-)
 * If less than MinRoomSize, it make MaxRoomSize same as MinRoomSize.
 * @default 3
 * 
 * @param ShowOuterWall
 * @desc Show outer wall. ('ON' is valid)
 * @default ON
 */

/*:ja
 * @plugindesc 自動マップ生成 ver1.10
 * 自動的にマップを生成しイベントを配置します。
 * @author サンシロ https://twitter.com/rev2nym
 * @version 1.10 2016/03/19 壁の高さ、部屋の大きさ、外壁表示を指定するプラグインパラメータを追加。
 * 壁を表示するスペースを確保するよう部屋と通路の生成規則を変更。
